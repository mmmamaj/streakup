...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const id = `story_${crypto.randomUUID()}`, createdAt = new Date(), story = { authorEmail: me, authorName: safe(req.body.authorName) || me.split("@")[0], authorUsername: safe(req.body.authorUsername) || me.split("@")[0], mediaUrl: safe(req.body.mediaUrl), mediaType: safe(req.body.mediaType), text: safe(req.body.text), createdAt: createdAt.toISOString(), expiresAt: new Date(createdAt.getTime() + 24 * 60 * 60 * 1000).toISOString() };
