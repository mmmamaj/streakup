...
export default async function handler(req, res) {
  ...
  const name = String(req.body?.name || "").trim(), username = String(req.body?.username || "").trim().replace(/^@/, ""), avatarUrl = String(req.body?.avatarUrl || "").trim();
  ...
  try {
    ...
    const updated = await fetch(`${API_BASE}/records/${encodeURIComponent(recordKey)}`, { method: "PUT", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ tipo: "login", data: { ...data, usuario: email, nome: name, username, avatarUrl, publicProfile } }) });
    if (!updated.ok) return res.status(updated.status).json({ error: "Não foi possível atualizar o perfil." });
    return res.status(200).json({ success: true, user: { name, username, avatarUrl, publicProfile, email } });
