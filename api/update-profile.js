...
export default async function handler(req, res) {
  ...
  try {
    ...
    const updated = await fetch(`${API_BASE}/records/${encodeURIComponent(recordKey)}`, { method: "PUT", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ tipo: "login", data: { ...data, usuario: email, nome: name, username, bio, avatarUrl, publicProfile } }) });
