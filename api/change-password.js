...
export default async function handler(req, res) {
  ...
  const key = process.env.DATABASE_API_KEY, email = String(req.body?.email || "").trim().toLowerCase(), currentPassword = String(req.body?.currentPassword || ""), newPassword = String(req.body?.newPassword || "");
  if (!key) return res.status(500).json({ error: "DATABASE_API_KEY não configurada." });
  if (!email || !currentPassword || !strongPassword(newPassword)) return res.status(400).json({ error: "Use uma senha forte: mínimo de 10 caracteres, com maiúscula, minúscula, número e símbolo." });
  try {
