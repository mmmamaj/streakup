...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/records`, {
      ...
      body: JSON.stringify({ chave: `login_${usuario}`, tipo: "login", data: { usuario, senha: password, nome, username: usuario.split("@")[0], avatarUrl: "" } })
    });
    ...
    return res.status(200).json({ success: true, message: "Conta salva na database.", user: { name: nome, email: usuario, username: usuario.split("@")[0], avatarUrl: "" } });
  } catch (error) {
    console.error(error);
