...
async function openComments(postId){
  ...
  sheet.hidden=false; sheet.innerHTML=`<div class="comment-sheet-inner"><button class="comment-close" type="button">×</button><h3>Comentários</h3><div class="comment-list">Carregando...</div><form class="comment-form"><input maxlength="500" placeholder="Adicione um comentário..."><button>Enviar</button></form></div>`;
  sheet.classList.add("show");
  const list=sheet.querySelector('.comment-list');
  try{ const r=await fetch(`/api/social?type=comments&postId=${encodeURIComponent(postId)}`,{cache:'no-store'}); const d=await r.json(); list.innerHTML=(d.comments||[]).map(c=>`<div class="comment-item"><strong>${esc(c.name)}</strong><span>@${esc(c.username)}</span><p>${esc(c.text)}</p></div>`).join('')||'<div class="muted">Ainda não há comentários.</div>'; }catch{list.innerHTML='<div class="muted">Não foi possível carregar os comentários.</div>'}
  sheet.querySelector('.comment-form').onsubmit=async e=>{e.preventDefault();const input=e.currentTarget.querySelector('input');const text=input.value.trim();if(!text)return;const r=await fetch('/api/social',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'comment',me:currentEmail,postId,text})});if(r.ok){input.value='';openComments(postId)}};
...
storiesBar?.addEventListener("click", async (event) => {
  ...
  const story = event.target.closest("[data-story-url]"); if (story) { const author=story.dataset.storyAuthor; storyQueue=[...document.querySelectorAll("[data-story-url]")].filter(x=>!author||x.dataset.storyAuthor===author).map(x=>({mediaUrl:x.dataset.storyUrl})); showStoryAt(0); }
});
