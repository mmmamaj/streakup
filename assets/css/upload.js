...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (buffer.length > 50 * 1024 * 1024) return res.status(413).json({ error: "O vídeo deve ter até 50 MB." });
    const form = new FormData();
    form.append("file", new Blob([buffer], { type: match[1] }), filename || "video.mp4");
    ...
    const id = payload.arquivo?.id;
    return res.status(201).json({ id, url: `/api/media?id=${encodeURIComponent(id)}` });
  } catch (error) {
    console.error(error);
...
export const config = { api: { bodyParser: { sizeLimit: "52mb" } } };
